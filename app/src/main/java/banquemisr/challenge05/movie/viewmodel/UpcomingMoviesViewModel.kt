...
class UpcomingMoviesViewModel @Inject constructor(private val getUpcomingMoviesUseCase: GetUpcomingMoviesUseCase) :
    ...
    private val TAG = "UpcomingMoviesViewModelTAG"
    public var upcomingMoviesStateFlow: MutableStateFlow<List<Movie>> =
    ...
    fun fetchUpcomingMovies() {
        viewModelScope.launch(Dispatchers.IO) {
            getUpcomingMoviesUseCase.invoke("en-US", 1)
                .collect {
                    var response = it
                    when (response) {
                        is Result.Success -> {
                            upcomingMoviesStateFlow.value = response.data
                        }
