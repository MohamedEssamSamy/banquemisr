...
class PopularMoviesViewModel @Inject constructor(private val getPopularMoviesUseCase: GetPopularMoviesUseCase) :
    ...
    private val TAG = "PopularMoviesViewModelTAG"

    public var popularMoviesStateFlow: MutableStateFlow<List<Movie>> = MutableStateFlow(emptyList())
        private set(value) {}
    ...
    fun fetchPopularMovies() {
        viewModelScope.launch(Dispatchers.IO) {
            getPopularMoviesUseCase.invoke("en-US", 1)
                .collect {
                    var response = it
                    when (response) {
                        is Result.Success -> {
                            popularMoviesStateFlow.value = response.data
                        }
